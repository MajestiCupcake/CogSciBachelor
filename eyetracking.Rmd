...
# individual test
```{r}
imo <- read.csv("IMOTION/001_11105.csv")
imo$SceneActive <- NA
imo$AOIActive <- NA
```
...
for (col in seq_along(imo)) {
  # Loop through rows
  if (startsWith(imo[24, col], "Screen")) {
      matches_scenes <- c(matches_scenes, list(col))
  }else {
  print(paste("no characters in row in coloumn:", col))
}
}

rows <- list()
for (col in seq(1,length(matches_scenes),1)){
  #print(col)
  colo <- matches_scenes[col][[1]]
  #print(colo)
  index <- which(imo[,colo] == 1)
  #print(index)
  rows <- c(rows,list(c(colo,index)))
}
```
...
imo <- df
for (coloumn in seq(1,length(rows),1)) {
  # Get the column index (first element in the list)
  col_index <- rows[coloumn][[1]][1]
  print(col_index)
  scene_name <- imo[24,col_index]
  print(scene_name)
  n_rows <- length(rows[coloumn][[1]][-1])
  spec_rows <- rows[coloumn][[1]][-1]
  
  for (i in spec_rows){
    imo[i,"SceneActive"] <- scene_name
  }
  
  imo <- imo[, -col_index, drop = FALSE]
...
## Merged from day 1
```{r}
df <- read.csv("IMOTION/ExportMerge.csv")
df$SceneActive <- NA
df$AOIActive <- NA
```
...
for (col in seq_along(df)) {
  # Loop through rows
  if (startsWith(df[25, col], "Screen")) {
      matches_scenes <- c(matches_scenes, list(col))
  }else {
  print(paste("no characters in row in coloumn:", col))
}
...
imo <- df
for (coloumn in seq(1,length(rows),1)) {
  mn <- length(rows)+1-coloumn
  # Get the column index (first element in the list)
  col_index <- rows[mn][[1]][1]
  print(col_index)
  scene_name <- imo[25,col_index]
  print(paste(scene_name, ' at ', col_index))
  n_rows <- length(rows[coloumn][[1]][-1])
  spec_rows <- rows[coloumn][[1]][-1]
...
write.csv(imo,"day1-collected-started.csv")
```

## load data
```{r}
df1 <- read.csv("ET_day1-collected-started.csv")
aoi <- df[ -c(1:76) ]
```

##load AOI data
```{r}
ao <- read.csv("work/EOTP/ET-folder/AOI_started_data/AggregatedAOImetrics.csv")
```

## start AOI dataframe cleaning
```{r}
df <- read.csv("Eye-tracking_data/001_11101.csv")
df1 <- read.csv("ET_day1-collected-started.csv")
df2 <- read.csv("Eye-tracking_data/ExportMerge.csv")
list.files(getwd())
```
...
## Make aoi readable
```{r}
aoi <- read_csv("AOI.csv")
```

```{r}
aoi <- read.csv("EOTP/AOI_grand_folder/AOI_MAGNIFISO.csv")
aoi <- AOI_MAGNIFISO
...
for (file in files){
  ...
  len <- nrow(temp)
  # remove everything from start row til og med #Data row
...
## crossferencing aoi dataframe with participant dataframes
### adding aoi's
```{r}
...
for (file in files){
  ...
  for (fixation in real_fix){
    ...
    for (i in seq(1,nrow(aois))){ #virker ikke
        ...
        #poly <- Polygons(list(Polygons(points), ID = "polygon"))
        
        # Create a SpatialPolygons object
        #polygon <- SpatialPolygons(list(poly))


        # Check if the point is inside the polygon
        #is_inside <- gContains(polygon, SpatialPoints(matrix(fixation_point, ncol = 2)))
...
```{r}
fixations_df <- read.csv("collected_fixations_dataframe_GRANDIOSA.csv")
```


# do the aoi thing just for the grandiosa dataframe try 1
```{r} 
...
## load data
```{r}
aoi <- read.csv("C:/Users/sarak/OneDrive - Aarhus universitet/26102021/Cog sci/5_semester/Bachelor/bachelor - github/analysis/AOI_MAGNIFISO.csv") #clean as in AOI clean up
...
```{r}
img_size <- read.csv("C:/Users/sarak/OneDrive - Aarhus universitet/26102021/Cog sci/5_semester/Bachelor/bachelor - github/img_size.csv")
...
make_heatmap <- function(participant, stimulus,task){
  ...
  img <- jpeg::readJPEG(paste0('C:/Users/sarak/OneDrive - Aarhus universitet/26102021/Cog sci/5_semester/Bachelor/bachelor - github/experiment_ready/experiment_images/out/', z$Stimuli[1],".jpg")) 
...
## draw
```{r, warning=FALSE}
animals <- c("hind_1","bison_5","ibex_3","horse_6","bison_6","hind_2")
for (animal in animals){
  for (task in unique(df$Task)){
    make_heatmap(i,animal,task)
    }
  }
```
...
#draw aoi
```{r}
new_aoi$AOI.Name[new_aoi$AOI.Name=="Head+neck"] <- "Head"
new_aoi$AOI.Name[new_aoi$AOI.Name=="Headdress"] <- "Head"
new_aoi$AOI.Name[new_aoi$AOI.Name=="Tail"] <- "Torso"
new_aoi$AOI.Name[new_aoi$AOI.Name=="Backlegs"] <- "Legs"
new_aoi$AOI.Name[new_aoi$AOI.Name=="Frontlegs"] <- "Legs"
new_aoi$x <- as.numeric(new_aoi$x)
new_aoi$y <- as.numeric(new_aoi$y)
new_aoi$y <- 1080 - new_aoi$y
...
make_aoi <- function(stimulus){
  z<- new_aoi[new_aoi$stim == stimulus,]
  img <- jpeg::readJPEG(paste0('/work/EOTP/out/', z$stim[1],".jpg")) 
  img <- grid::rasterGrob(img, width=unit(1, "npc"), height = unit(1,"npc"), interpolate = FALSE)
    ymx <- (1080-img_size$ypix[img_size$stim==stimulus])/2+img_size$ypix[img_size$stim==stimulus]
    xmx <- (1920-img_size$xpix[img_size$stim==stimulus])/2+img_size$xpix[img_size$stim==stimulus]
    ym <- (1080-img_size$ypix[img_size$stim==stimulus])/2
...
p <- ggplot(z, aes(x = x, y = y)) +
  annotation_custom(img, xmin = xm, xmax = xmx, ymin = ym, ymax = ymx)+
  geom_polygon(aes(fill = AOI.Color, group = AOI.Name), alpha = 0.5) +
  scale_fill_manual(values = c("#6E0955", "#009BBB", "#7E5738")) +
      xlim(xmin = 0, xmax = 1920) +ylim(ymin = 0, ymax = 1080) +
        labs(title=NULL,x=NULL,y=NULL)+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank())+theme(legend.position = "none")+theme(axis.text.x=element_blank(), axis.ticks.x=element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank())+guides(fill = guide_legend(override.aes = list(title = NULL))) + guides(alpha = guide_legend(override.aes = list(title = NULL)))
  print(p)
ggsave(paste0("/work/EOTP/out/aoi/",stimulus,"_aoi.jpg"),width = 16, height = 9, units = "in")
}
```

```{r}
animals <- c("bison_1","hind_2","horse_6","ibex_5")
for (animal in animals){
  make_aoi(stimulus=animal)
}
```